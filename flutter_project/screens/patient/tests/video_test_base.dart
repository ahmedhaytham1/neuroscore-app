...
class _VideoTestScreenState extends State<VideoTestScreen>
      ...
      child: Container(
        ...
        child: ExpansionTile(
          ...
          children: [
            ...
            const SizedBox(height: 14),
            if (r.chartData.isNotEmpty) ...[
              _buildChartPreview(r),
              const SizedBox(height: 14),
            ],
            // Features list
            ...r.features.entries.take(20).map((e) => Padding(
  ...
  Widget _buildChartPreview(AIAnalysisResult r) {
    final firstSeries = r.chartData.entries.firstWhere(
      (e) => e.value is List && (e.value as List).isNotEmpty,
      orElse: () => const MapEntry('', []),
    );
    final values = (firstSeries.value as List)
        .whereType<num>()
        .map((e) => e.toDouble())
        .take(40)
        .toList();
